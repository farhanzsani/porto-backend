<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-5">
            ...
            @if ($errors->get('email'))
                <ul class="mt-2 space-y-1">
                    @foreach ($errors->get('email') as $message)
                        <li class="text-error-600 text-xs dark:text-error-400">{{ $message }}</li>
                    @endforeach
                </ul>
            @endif
        </div>
        ...
        <div class="flex items-center justify-between">
            <label class="inline-flex cursor-pointer items-center gap-2 text-sm font-normal text-gray-700 select-none dark:text-gray-400">
                <input id="remember_me" type="checkbox" name="remember" checked class="size-5 rounded border-gray-300 text-brand-500 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900">
                Remember me
            </label>
        ...
        <button type="submit" class="flex w-full items-center justify-center rounded-lg bg-brand-500 px-4 py-3 text-sm font-medium text-white shadow-theme-xs transition-colors hover:bg-brand-600">
            Sign In
        </button>
    </form>
</x-guest-layout>